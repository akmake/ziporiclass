...
export default function ChatPage() {
  ...
  const { selectContact, activeContactId, contacts } = useChatStore();
  ...
  const handleSelectContact = (contact) => {
    ...
    setIsMobileListVisible(false); // במובייל עבור לצ'אט
  };
        ...
        {activeContact ? (
          ...
          <div className="flex-1 flex items-center justify-center text-slate-400 relative z-10">
            <div className="text-center bg-white/80 p-8 rounded-2xl shadow-sm backdrop-blur-sm">
              ...
              <p>בחר איש קשר מהרשימה בצד ימין כדי להתחיל שיחה</p>
            </div>
          </div>
        )}
      </div>
